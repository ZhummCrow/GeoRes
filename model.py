...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # 计算注意力权重
        attn_scores = self.attention(x)  # [batch_size, seq_len, heads]
        attn_scores = attn_scores.transpose(1, 2)  # [batch_size, heads, seq_len]
        mask = mask.unsqueeze(1)  # [batch_size, 1, seq_len]
        attn_scores = attn_scores.masked_fill(mask == 0, -1e9)
        
        # 计算softmax注意力权重
        attn_weights = F.softmax(attn_scores, dim=-1)  # [batch_size, heads, seq_len]
        
        # 应用注意力权重进行加权求和
        pooled = torch.bmm(attn_weights, x) # [batch_size, heads, input_dim]
        
        return pooled, attn_weights


class GPSite(nn.Module): # Geometry-aware Protein Sequence-based binding site predictor
    def __init__(self, node_input_dim, edge_input_dim, hidden_dim, num_layers, dropout, augment_eps,task_list):
        ...
        self.heads=4
        self.attention_pooling = AttentionPooling(hidden_dim,hidden_dim,heads=self.heads)

        self.task_list = task_list
        for task in self.task_list:
            self.add_module("FC_reg_{}1".format(task), nn.Linear(hidden_dim*(self.heads+2), hidden_dim, bias=True))
            self.add_module("FC_reg_{}2".format(task), nn.Linear(hidden_dim, 1, bias=True))
            
            self.add_module("FC_class_{}1".format(task), nn.Linear(hidden_dim*(self.heads+2), hidden_dim, bias=True))
            self.add_module("FC_class_{}2".format(task), nn.Linear(hidden_dim, 3, bias=True))      

    def inference(self, wt_data, mut_data):
        X = mut_data.X
        edge_index = mut_data.edge_index
        ...
        wt_h_V = self.Graph_encoder(wt_h_V, edge_index, h_E, batch) 
        
        mut_h_V = torch.cat([mut_h_V, h_V_geo], dim=-1)
        mut_h_V = self.Graph_encoder(mut_h_V, edge_index, h_E, batch) 
        
        delta_h_V = mut_h_V - wt_h_V  

        delta_h_V_mean_max_min = torch.cat([
            global_max_pool(delta_h_V,batch),
            global_mean_pool(delta_h_V,batch),
        ],dim=-1)
        
        batch_delta_h_V,mask = to_dense_batch(delta_h_V,batch) 
        delta_h_V,att_weight = self.attention_pooling(batch_delta_h_V,mask)
        delta_h_V = delta_h_V.reshape(delta_h_V.shape[0],-1) 
        
        delta_h_V = torch.cat([delta_h_V,delta_h_V_mean_max_min],dim=-1)
         
        # delta
        delta_reg_outputs = []
        delta_class_outputs=[]
        for task in self.task_list:
            emb_reg = F.elu(self._modules["FC_reg_{}1".format(task)](delta_h_V))
            reg_out = self._modules["FC_reg_{}2".format(task)](emb_reg)
            delta_reg_outputs.append(reg_out)
            
            emb_cls = F.elu(self._modules["FC_class_{}1".format(task)](delta_h_V))
            cls_out = self._modules["FC_class_{}2".format(task)](emb_cls) 
            delta_class_outputs.append(cls_out.unsqueeze(1)) 
            
        delta_reg_outputs = torch.cat(delta_reg_outputs, dim=1) 
        delta_class_outputs = torch.cat(delta_class_outputs, dim=1)
        
        return delta_reg_outputs,delta_class_outputs # [B, num_tasks] [B, num_tasks, 3]